        ...
        regionSelection: function (options) {
            ...
            return this.each(function () {
                ...
                function load_select(el_id, parent_code, option_name, is_bind_change, selected_code) {
                    ...
                    var sl_item = $("<select id='" + el_id + "' name='" + el_id + "' class='" + o.class_name + " region_seclect'></select>");
                    var op1 = $("<option  value=''>" + option_name + "</option>");
                    sl_item.append(op1);
                    //请求数据
                    $.post(o.data_request_url, { parentId: parent_code }, function (res) {
                        if (res && res.code == '200') {
                            var datas = res.data;
                            ...
                            if (datas.length > 0) {
                                ...
                                for (var i = 0; i < datas.length; i++) {
                                    var item = datas[i];
                                    var opt = '';
                                    if (selected_code == item.regionCode) {
                                        opt = $("<option  value='" + item.regionCode + "' selected='selected'>" + item.regionName + "</option>");
                                    }
                                    else {
                                        opt = $("<option  value='" + item.regionCode + "'>" + item.regionName + "</option>");
                                    }
                                    sl_item.append(opt);
                                }
                            }
