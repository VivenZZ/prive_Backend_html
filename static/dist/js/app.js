...
var AppButton = {
    ...
    lock: function (ele, msg) {
        AppButton._text = ele.html();

        var html = [];
        html.push('<i class="fa fa-spinner fa-spin"></i>');
        if (!msg)
            html.push('正在提交信息，请稍等...');

        ele.html(html.join(''));
        ele.attr("disabled", true);
    },
    unlock: function (ele) {
        ele.html(AppButton._text);
        ele.attr("disabled", false);
    }
};

//ajax提交
var richAjax = function (data, url, btnLock, pageLock) {
    var _ = {
        Data: data,
        Action: url,
